...
TEST(TestFSM, TestNumberFSM) {
    ...
    {
        const std::array inputString{'f', '\0'};
        auto result = fsm.analyze(inputString);
        EXPECT_FALSE(result.has_value());
    }
    {
...
TEST(TestFSM, TestTempExpr) {
    ...
    {
        const std::array inputString{'e', 'a', 'r', 'l', 'y', '-', '2', '0', '0', '0', 's', '\0'};
        auto result = fsm.analyze(inputString);
        EXPECT_TRUE(result.has_value());
        const auto& [finalState, value] = *result;
    ...
    {
        const std::array inputString{'l', 'a', 't', 'e', '-', '1', '9', '9', '0', 's', '\0'};
        auto result = fsm.analyze(inputString);
        EXPECT_TRUE(result.has_value());
        const auto& [finalState, value] = *result;
        EXPECT_EQ(finalState, tempExpr);
    ...
    {
        const std::array inputString{'m', 'i', 'd', '-', '1', '9', '9', '0', 's', '\0'};
        auto result = fsm.analyze(inputString);
        EXPECT_TRUE(result.has_value());
        const auto& [finalState, value] = *result;
        EXPECT_EQ(finalState, tempExpr);
