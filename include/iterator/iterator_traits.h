...
private:
    ...
    template<class T>
    struct DifferenceTypeGetter<T, std::void_t<>>
    {
    private:
        template<class U>
        static auto probe(int) -> std::ptrdiff_t;

        template<class U>
        static auto probe(long) -> decltype(std::declval<U>() - std::declval<U>());

    public:
        using Type = decltype(probe<T>(0));   // 0  → int, 1L → long
    };
